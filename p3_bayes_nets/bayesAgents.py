...
def constructBayesNet(gameState):
    ...
    '========Populate `obsVars` using the procedur above========'
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
...
def fillYCPT(bayesNet, gameState):
    ...
    "====Using setProbability to assign value to the four condition. Values can be seen in layout.py===="
...
def fillObsCPT(bayesNet, gameState):
    ...
    cnt = 0
    for housePos in gameState.getPossibleHouses():  # 4 possible position
        #record the current house
        Curr_house = BOTTOM_LEFT_VAL
        if cnt == 1:
            Curr_house = TOP_LEFT_VAL
        elif cnt == 2:
            Curr_house = BOTTOM_RIGHT_VAL
        elif cnt == 3:
            Curr_house = TOP_RIGHT_VAL
        cnt+=1;
        # each position we should have 7 observation
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVarFactor = bn.Factor(
                [obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())

            # for each observation, we should assgin 3 types
            for assignment in obsVarFactor.getAllPossibleAssignmentDicts():
                #order should be strictly obeyed, for the sake of ghost and food house are the same.
                if (assignment[FOOD_HOUSE_VAR] != Curr_house and assignment[GHOST_HOUSE_VAR] != Curr_house):
                    if assignment[obsVar] == NO_OBS_VAL:
                        obsVarFactor.setProbability(assignment, 1)
                    else:
                        obsVarFactor.setProbability(assignment, 0)  
                elif (assignment[GHOST_HOUSE_VAR] == assignment[FOOD_HOUSE_VAR]) or \
                    (assignment[FOOD_HOUSE_VAR] == Curr_house):
                    if assignment[obsVar] == NO_OBS_VAL:
                        obsVarFactor.setProbability(assignment, 0)
                    elif assignment[obsVar] == RED_OBS_VAL:
                        obsVarFactor.setProbability(assignment, PROB_FOOD_RED)
                    else:
                        obsVarFactor.setProbability(assignment, 1-PROB_FOOD_RED)
                elif (assignment[GHOST_HOUSE_VAR] == Curr_house):
                    if assignment[obsVar] == NO_OBS_VAL:
                        obsVarFactor.setProbability(assignment, 0)
                    elif assignment[obsVar] == RED_OBS_VAL:
                        obsVarFactor.setProbability(assignment, PROB_GHOST_RED)
                    else:
                        obsVarFactor.setProbability(assignment, 1-PROB_GHOST_RED)
                else:#redundancy part
                    if assignment[obsVar] == NO_OBS_VAL:
                        obsVarFactor.setProbability(assignment, 1)
                    else:
                        obsVarFactor.setProbability(assignment, 0)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    CPT = inference.inferenceByVariableElimination(bayesNet,FOOD_HOUSE_VAR,evidence,eliminationOrder)
    value = 0
    assignment = {}
    for temp_assignment in CPT.getAllPossibleAssignmentDicts():
        if CPT.getProbability(temp_assignment) > value:
            value = CPT.getProbability(temp_assignment)

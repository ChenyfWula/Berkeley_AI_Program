...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # grab all factors where we know the evidence variables (to reduce the size of the tables)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        #for the time being, all of the factors haven't been joined
        factors_not_joined = currentFactorsList
        
        #based on the elimination order to eliminate, first join all the factors that has the elimintate var
        for eliminate_var in eliminationOrder:
            factors_not_joined, factors_joined = joinFactorsByVariable(factors_not_joined, eliminate_var)
            
            #if != 1, it means that an eliminate variable is in the unconditioned variable, and it's the case that the
            #eliminate_var joined itself.
            #so sum(eliminate) it and update the factors that haven't been joined
            if len(factors_joined.unconditionedVariables()) != 1:
                factor = eliminate(factors_joined, eliminate_var)
                factors_not_joined.append(factor)
                
        factor = joinFactors(factors_not_joined)
